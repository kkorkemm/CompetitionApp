...
namespace CompetitionApp.Pages.CompetitiorPages
{
    ...
    public partial class CompetitorUsersPage : Page
    {
        ...
        {
            InitializeComponent();
            ListUsers.ItemsSource = CompetitionDBEntities.GetContext().User.Where(p => p.CompetiotionID == CompetitionDBEntities.currentCompettion.ID && p.UserRoleID == 1 && p.ID != CompetitionDBEntities.currentUser.ID).ToList();
        }
